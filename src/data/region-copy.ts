  ...
  return [
    {
      ...
      paragraphs: [
        `Heizöl Deutschland liefert in alle ${num(state.cityCount)} Städte und Gemeinden sowie ${num(state.plzCount)} Postleitzahl-Bereiche in ${state.name}. Der aktuelle Heizölpreis beträgt ab ${price()} pro 100 Liter (Stand: ${todayStamp()}). Bei einer durchschnittlichen Bestellung von ${num(SAMPLE_LITERS)} Litern liegt der Gesamtpreis bei ca. ${total()} — kostenlose Lieferung inklusive.`,
        `Heizölpreise in ${state.name} schwanken täglich je nach Rohölpreis, Dollar-Kurs und saisonaler Nachfrage. Wählen Sie unten Ihre Stadt oder berechnen Sie Ihren Preis direkt mit Ihrer Postleitzahl. Mit über ${REVIEW_COUNT} zufriedenen Kunden deutschlandweit und ${REVIEW_SCORE}/5 Sternen gehört Heizöl Deutschland zu den beliebten Heizöl-Anbietern Deutschlands.`,
      ],
    },
    {
      heading: `Heizölpreis pro Liter in ${state.name} — was kostet 1 Liter?`,
      paragraphs: [
        `Der Heizölpreis pro Liter in ${state.name} liegt heute bei ca. ${perLiter()} pro Liter — das entspricht ${price()} pro 100 Liter. Im deutschen Heizöl-Markt ist die Angabe €/100 L Standard, da Heizöl nur in größeren Mengen verkauft wird; bei Heizöl Deutschland beträgt die Mindestbestellmenge ${num(MIN_LITERS)} Liter.`,
    ...
    },
    {
      ...
      paragraphs: [
        `Beim Heizöl bestellen in ${state.name} ist die Liefermenge entscheidend für den Preis pro 100 Liter: Je größer die Bestellmenge, desto günstiger der €/100-L-Preis. Die Mindestbestellmenge bei Heizöl Deutschland beträgt ${num(MIN_LITERS)} Liter. Die Lieferung erfolgt deutschlandweit innerhalb von ${DELIVERY_DAYS} Werktagen und ist im Preis enthalten.`,
...
export function cityBlocks(city: City, state: State): CopyBlock[] {
  ...
  const intro = [
    `Heizöl Deutschland liefert nach ${city.name} und in die umliegenden Orte im Postleitzahl-Bereich ${city.plz}. Der aktuelle Heizölpreis für ${city.name} beträgt ab ${price()} pro 100 Liter (Stand: ${todayStamp()}).`,
    `Heizöl für ${withState(city, state)} bestellen Sie bei Heizöl Deutschland tagesaktuell ab ${price()} pro 100 Liter (Stand: ${todayStamp()}) — geliefert wird im PLZ-Gebiet ${city.plz} und in der gesamten Region.`,
  ...
  return [
    {
      ...
      paragraphs: [
        ...
        `${city.name}${city.district ? ` im Landkreis ${city.district}` : ""} hat rund ${num(city.population)} Einwohner. Mit über ${REVIEW_COUNT} zufriedenen Kunden und ${REVIEW_SCORE}/5 Sternen gehört Heizöl Deutschland zu den beliebten Heizöl-Anbietern Deutschlands.`,
      ],
    },
    {
...
export function cityFaq(city: City, state: State): { q: string; a: string }[] {
  return [
    ...
    {
      q: `Liefert Heizöl Deutschland nach ${city.name}?`,
